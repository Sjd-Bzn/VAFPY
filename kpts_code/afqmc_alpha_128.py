...
def get_alpha_full(trial,h2,q_list):
    out=np.zeros([num_k*num_electrons_up,num_orb*num_k,num_g],dtype=np.complex128)
...
def get_alpha_mul(trial,h2,h2_dagger,q_list,minus_q):
    aat = np.zeros([num_k*num_electrons_up,num_orb*num_k,num_k*num_electrons_up,num_orb*num_k],dtype=np.complex128)
    print(aat.shape)
...
del(h1)
del(H1)
hamil.two_body = np.array(read_datafile(input_file_two_body_hamil))
...
np.save('alpha_128.npy',alp)
#print('alpha done')
alp_t = get_alpha_full_t(PSI_T_up,h2_t,ql,m_q)
print('alpha_t done')
np.save('alpha_t_128.npy',alp_t)
#alp = np.load('alpha_128.npy')
#alp_t = np.load('alpha_t_128.npy')
...
np.save('H2_af_128.npy',h2_af_MF_sub)
