...
from afqmc_INPUT_H2_11 import *
from opt_einsum import contract
from opt_einsum import contract_expression

#from afqmc_alpha import expr_exch_3
#from afqmc_alpha import expr_hart
from afqmc_alpha_11 import expr_h2_e
from afqmc_alpha_11 import expr_h2_o
from afqmc_alpha_11 import expr_fb_e
from afqmc_alpha_11 import expr_fb_o
...
from afqmc_alpha_11 import expr_hart_new_new
from afqmc_alpha_11 import expr_exch_new_new
...
#@profile
def update_hyb(trial_0,trial,walker_mat,walker_weight,q_list,h_0,h_1,d_tau,e_0,exp_h1,exp_h1_half):
    NG = num_g
    ...
    fb_e_Q[abs(fb_e_Q)>1] = 1.
    fb_o_Q[abs(fb_o_Q)>1] = 1.
    ...
    for i in range(0,NUM_WALKERS):
        ...
        #    addend = h@addend/(j + 1)
        #new_walker_mat[i] = np.exp(d_tau*(-h_0+e_0))*new_walker_mat[i]
        #prop_taylor = np.exp(D_TAU*(-h_0+e_0))*exp_Taylor(h)
        #prop_S1 = np.exp(D_TAU*(-h_0+e_0))*exp_h1*exp_Taylor(SQRT_DTAU*1j*h_2[:,:,i])
        #prop_S2 = np.exp(D_TAU*(-h_0+e_0))*exp_h1_half@exp_Taylor(SQRT_DTAU*1j*h_2[:,:,i])@exp_h1_half
        prop_S2 = exp_h1_half@exp_Taylor(SQRT_DTAU*1j*h_2[:,:,i])@exp_h1_half
        #new_walker_mat[i] = prop_taylor @ walker_mat[i]
        #new_walker_mat[i] = prop_S1 @ walker_mat[i]
        new_walker_mat[i] = prop_S2 @ walker_mat[i] 
        #print('S1 ? Taylor -> ', np.allclose(new_state_t,new_state_s1,atol=1e-3))
        #print('S1 ? S2 -> ', np.allclose(new_state_s2,new_state_s1,atol=1e-3)) 
...
def update_walker(trial_0,trial,walker_mat,walker_weight,q_list,h_0,h_1,d_tau,e_0,selected_update_method,exp_h1,exp_h1_half):
    return selected_update_method(trial_0,trial,walker_mat,walker_weight,q_list,h_0,h_1,d_tau,e_0,exp_h1,exp_h1_half)
