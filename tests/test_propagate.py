...
def test_explicit(make_constants):
    ...
    actual = np.average((1 - overlap.real) / tau)
    print(f"{actual=}")
    order1 = np.einsum("nmg,gw,mi->nmw", constants.L, np.eye(constants.number_g), constants.trial_det)
    order2 = np.einsum("nmg,gw,miw->nmw", constants.L, np.eye(constants.number_g), order1)
    overlap = determinant.overlap_trial(constants, order2)
    print(f"{overlap=}")
