...
from vafpy_sobol import input_reader
...
import vafpy_sobol.functions as new
...
#    update_method = set_update_method(afqmc.UPDATE_METHOD)
    ...
    # Each MPI rank gets its own Sobol seed so walkers don't share the same field sequence
    sobol_seed = int(seed) + rank * 7919 + afqmc.sobol_seed
    ...
    config = new.Configuration(
        ...
        backend=backend,
        use_sobol=afqmc.use_sobol,
        sobol_seed=sobol_seed,
    )
    if afqmc.first_cpu:
        if afqmc.use_sobol:
            print(f"RNG mode : Sobol RQMC (d=2*{afqmc.num_g}={2*afqmc.num_g}, "
                  f"N={afqmc.NUM_WALKERS}, base seed={afqmc.sobol_seed})")
        else:
            print("RNG mode : pseudo-random (NumPy / JAX backend)")
