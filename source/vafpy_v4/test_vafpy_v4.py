...
def test_multi_k_consistency():
    ...
    # Build block-diagonal (16, 16, 36*2 = 72). Each k-block sits in its
    # own Q slice so the Q-list is internally consistent and the
    # compressed runtime in v4 sees the data at the expected indices.
    nb, _, ng = h2_one.shape
    h2_two = np.zeros((nb * 2, nb * 2, ng * 2), dtype=h2_one.dtype)
    h2_two[:nb, :nb, :ng] = h2_one     # (k1=1, k2=1) lives in Q=1 slice
    h2_two[nb:, nb:, ng:] = h2_one     # (k1=2, k2=2) lives in Q=2 slice
    np.save("/tmp/H2_two.npy", h2_two)

    # Each block's Q-label matches the slice where its data lives.
    ...
    h2_2 = new.obtain_H2(cfg2, "/tmp/H2_two.npy", q_list=ql2)
