...
#    theta_mat = np.array([theta(trial, walker) for walker in walker_mat])
#
#    # Precompute feedback terms once
...
#    x_e_Q = np.random.randn(NG, NUM_WALKERS)
#    x_o_Q = np.random.randn(NG, NUM_WALKERS)
#
#    # Compute interaction Hamiltonian terms
#    h_2 = expr_h2_e(x_e_Q - fb_e_Q.T) + expr_h2_o(x_o_Q - fb_o_Q.T)
#
#    # Main walker update loop
#    for i in range(NUM_WALKERS):
#       # h = h_1 + SQRT_DTAU * 1j * h_2[:, :, i]
#        prop_S2 = e_0 * h1_exp_half @ exp_Taylor(SQRT_DTAU * 1j * h_2[:, :, i]) @ h1_exp_half
...
#        new_walker_weight[i] = (
#            abs(ovrlap_ratio * np.exp(np.dot(x_e_Q[:, i], fb_e_Q[i]) - np.dot(fb_e_Q[i], fb_e_Q[i] / 2)) *
#                np.exp(np.dot(x_o_Q[:, i], fb_o_Q[i]) - np.dot(fb_o_Q[i], fb_o_Q[i] / 2))) 
#            * max(0, np.cos(alpha)) * walker_weight[i]
#        )
#
...
def reortho_qr(walker_matrix):
    """
    Uses QR decomposition to reorthogonalize the orbitals in a single walker.
    Optimized to avoid unnecessary computation.
    """
    return np.linalg.qr(walker_matrix, mode='reduced')[0]  # Only return Q
