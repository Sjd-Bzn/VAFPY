...
if first_cpu:
    ...
    #print('H2.npy shape -> ', hamil.two_body.shape)
...
#
    ...
    if abs(energy_new.imag)<abs(trsh_imag) and abs(energy_new.real-e_hf.real)<abs(trsh_real): #ratio*(energy.real+en_const))):# and (abs((energy.real-e_hf.real)/(en_const+e_hf.real))<MAX_ACC_VAL):
        if first_cpu:
