...
#    return new_indices
#
#def rebalance_global(walkers_mats_up, walkers_weights):
#    """
#    Global resampling using Gather → Resample → Scatter strategy.
#    
#    Assumes all ranks have equal number of walkers (local_n).
#    
#    Parameters:
#    - walkers_mats_up: ndarray [local_n, N_orb, N_elec]
#    - walkers_weights: ndarray [local_n]
#    - seed: random seed for reproducibility
#    
#    Returns:
#    - new_mats_up: ndarray [local_n, N_orb, N_elec] (after resampling)
#    - new_weights: ndarray [local_n] (reset to uniform)
#    """
#    comm = MPI.COMM_WORLD
#    rank = comm.Get_rank()
#    size = comm.Get_size()
#
#    local_n = walkers_mats_up.shape[0]
#    N_orb, N_elec = walkers_mats_up.shape[1], walkers_mats_up.shape[2]
#    total_n = local_n * size
#
#    # === Step 1: Gather weights and walker matrices on rank 0 ===
#    all_weights = None
#    if rank == 0:
#        all_weights = np.empty(total_n, dtype=np.complex128)
#    comm.Gather(walkers_weights, all_weights, root=0)
...
#    comm.Gather(walkers_mats_up, all_mats_up, root=0)
#
#    # === Step 2: Resample on rank 0 ===
#    if rank == 0:
#        #rng = np.random.default_rng(seed)
#        indices = rebalance_comb(all_weights)
#        resampled_mats_up = all_mats_up[indices]
#    else:
#        resampled_mats_up = None
#
#    # === Step 3: Scatter resampled matrices back to all ranks ===
#    new_mats_up = np.empty((local_n, N_orb, N_elec), dtype=np.complex128)
#    comm.Scatter(resampled_mats_up, new_mats_up, root=0)
#
#    # === Step 4: Reset weights ===
#    new_weights = np.ones(local_n, dtype=np.complex128)
#
#    return new_mats_up, new_weights
