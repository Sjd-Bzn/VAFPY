...
from afqmc_alpha_ref import expr_h2_e
from afqmc_alpha_ref import expr_h2_o
...
from afqmc_alpha_ref import expr_hart_new_new
from afqmc_alpha_ref import expr_exch_new_new
from afqmc_alpha_ref import expr_fb_e
from afqmc_alpha_ref import expr_fb_o
...
def update_hyb(trial_0,trial,walker_mat,walker_weight,q_list,h_0,h_1,d_tau,e_0,h1_exp_half,propagator):
    ...
    fb_e_Q[abs(fb_e_Q) >= 1] = 0.0
    fb_o_Q[abs(fb_o_Q) >= 1] = 0.0
...
########       Different propagators Taylor, S1, and S2
            ...
            prop_S2 = np.exp(d_tau * (h_0 )) * h1_exp_half@exp_Taylor(SQRT_DTAU*1j*h_2[:,:,i])@h1_exp_half
...
####new_weight
        ...
        a = (ovrlap_ratio*((np.exp( np.dot(x_e_Q[:,i],fb_e_Q[i])-np.dot(fb_e_Q[i],fb_e_Q[i]/2))*np.exp(np.dot(x_o_Q[:,i], fb_o_Q[i])-np.dot(fb_o_Q[i],fb_o_Q[i]/2))))*np.exp(d_tau*e_0))

        reweighting = abs(a)
    #    print('reweighting shape', reweighting)

     #   print('e_0 shape', e_0)
        if reweighting < 10: a = a
        else:
            a = 0
            num_rare_event += 1
...
####new_weight
        ...
        new_walker_weight[i] = abs(a)* max(0,np.cos(alpha))*walker_weight[i]
        ...
        #print("reweight", reweighting)
    return new_walker_mat, new_walker_weight, num_rare_event
