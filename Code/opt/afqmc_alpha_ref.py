...
def mean_field(  num_elec, num_band):
...
def gen_A_e_o_full(trial_0, trial, q_list):
    '''
    It generates A_e.
    '''
    h2 = np.array(read_datafile(input_file_two_body_hamil),dtype=np.complex128)
    ...
    for Q in range(1,num_k+1):
        ...
        for K1,K2 in K1s_K2s:
            for g in range(h2_shape[2]):
                for r in range(num_orb):
                    avg_A_mat[(K1-1)*num_orb+r][(K2-1)*num_orb+r][g]=avg_A_vec_Q[g]
    h2_mf = h2-avg_A_mat/num_electrons_up/2/num_k
    a_o = (h2_mf - np.einsum('ijG->jiG', h2_mf.conj()))*1j/2
    a_e = (h2_mf + np.einsum('ijG->jiG', h2_mf.conj()))/2
    return a_e,  a_o

def gen_A_o_full(trial_0, trial2, q_list):
    '''
    It generates A_o.
    '''
    avg_A_mat = np.zeros_like(h2)
...
def h_self():
    '''
    It computes self energy
    '''
    h2 = np.array(read_datafile(input_file_two_body_hamil),dtype=np.complex128)
    h2_t = np.einsum('prG->rpG', h2.conj())
    self = -contract('ijG,jkG->ik',h2, h2_t)/2
    return self
...
# Mean_field subtraction
#h2_af_MF_sub = A_af_MF_sub(PSI_T_up_0, PSI_T_up, hamil.two_body, ql)
h_self = h_self() #-contract('ijG,jkG->ik',hamil.two_body, np.einsum('prG->rpG', hamil.two_body.conj()))
#h2_t)/2#/num_k
#hamil_MF.zero_body =  H_0_mf(PSI_T_up_0,PSI_T_up,hamil.two_body,h2_t,ql)
print("h self",h_self)
#exit()
hamil_MF.one_body = H_1_mf(PSI_T_up_0,PSI_T_up,ql,hamil.one_body)
print("hamil_MF one",hamil_MF.one_body)
#exit()
L_0 = mean_field( num_electrons_up, num_orb)
...
H1_self_half_exp = expm(-D_TAU * (hamil_MF.one_body+h_self)/2)
hamil_MF.two_body_e,  hamil_MF.two_body_o= gen_A_e_o_full(PSI_T_up_0, PSI_T_up,  ql)
...
def ALP(trial):
    h2 = np.array(read_datafile(input_file_two_body_hamil),dtype=np.complex128)
    alp = np.einsum('pi, prg -> irg', trial , h2)
    alp_t = np.einsum('pi, rpg -> irg', trial, h2.conj())
...
#ALPHA_E = contract('ip,prG->irG',PSI_T_up.T, gen_A_e_full(PSI_T_up_0, PSI_T_up, hamil.two_body, ql))
ALPHA_E = contract('ip,prG->irG',PSI_T_up.T, hamil_MF.two_body_e)
ALPHA_O = contract('ip,prG->irG',PSI_T_up.T, hamil_MF.two_body_o)
print("Alpha E",ALPHA_E.shape)
print("Alpha O",ALPHA_O.shape)
print("h2_e",hamil_MF.two_body_e.shape)
print("h2_o",hamil_MF.two_body_o.shape)


#np.save('a_e_intl.npy',ALPHA_E)
#ALPHA_O = contract('ip,prG->irG',PSI_T_up.T, gen_A_o_full(PSI_T_up_0, PSI_T_up, hamil.two_body, ql))
