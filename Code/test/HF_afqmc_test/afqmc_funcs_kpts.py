...
def measure_E_gs(trial,weights,walkers,h_1):#,alpha_full,alpha_full_t):#,comm):
    ...
    print(2*expr_hart_new_new(walkers, walkers)[0])
    print(expr_exch_new_new(walkers, walkers)[0])
    e_locs=e1+har_list-exch_list
    val=0
    for e_loc, weight in zip(e_locs, weights):
        val+=e_loc*weight
...
def propagator_fp(h_mf,h_1,d_tau,e_0):
    ...
    return np.exp(-d_tau*(h_0-e_0))*exp_Taylor(h_1)
...
def average_Hamil(h_mf, h_1, psi_up, walker, n_w, d_tau, e_0):
    '''
    It computes 1/n_w Re { sum_{w} ( 1 - det(psi_up_dagger . propog . psi_up ) x det(psi_down_dagger . propog . psi_down ) ) / d_tau }.
    '''
    #avg = 0
    propag = propagator_fp(h_mf, h_1, d_tau, e_0)
    result = np.dot(propag,walker)
    result = np.dot(psi_up.conj().transpose(),result)
    det_up = (np.linalg.det(result))**2
    return ((1-det_up)/d_tau).real
