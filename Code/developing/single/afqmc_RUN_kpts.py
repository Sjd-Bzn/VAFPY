...
h2 = H2.astype(np.complex64)#np.moveaxis(H2, 0, -1) 
hamil.two_body = h2
...
#print('hamil_MF.two_body_e.dtype',hamil_MF.two_body_e.dtype)
#print('hamil_MF 2 body',h2_af_MF_sub.dtype)
#
if first_cpu:
...
while (j<NUM_STEPS+1):
    ...
    if j%SAMP_FREQ==0:
       # t1=time()
        #a=  measure_E_gs_double(PSI_T_up,walkers.weights,walkers.mats_up,hamil.one_body,e_hf)#,comm)#-2*num_electrons_up*num_electrons_up*num_k*fsg
        #time_double = time() - t1
        #print(f"energy time double: {time_double:.6f} second")
        t2=time()
        a=  measure_E_gs(PSI_T_up.astype(np.float32),walkers.weights,walkers.mats_up,hamil.one_body,e_hf)
        energy_time = time() - t2
...
if first_cpu:
    with open('output.txt', 'a') as o:
        print(file=o)
        print('Calculating ground state energy...', file=o)
        print(file=o)
        print('E_gs_afqmc = ', np.mean(data[st:en]), '+-', np.sqrt(max(a[1])), file=o)
        print('total runtime = ', -start_time+time(), file=o)
